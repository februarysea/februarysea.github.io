...
export function getCurrentTimeInShanghai(): Date {
  // Create a date object with the current UTC time
  const now = new Date();

  // Convert the UTC time to Shanghai's time (UTC+8)
  const offsetShanghai = 8;
  now.setHours(now.getUTCHours() + offsetShanghai);

  return now;
}

export function formatTimeForShanghai(date: Date): string {
